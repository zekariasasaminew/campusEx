...
export const updateListingSchema = z
  .object({
    ...
    condition: z
      .enum(CONDITIONS as [string, ...string[]])
      .nullable()
      .optional(),
    price_cents: z.number().int().min(0).nullable().optional(),
    is_free: z.boolean().optional(),
    location: z.string().trim().max(VALIDATION_RULES.location.max).nullable().optional(),
  })
  .refine(
    (data) => {
      // If is_free is true, price_cents must be null
      if (data.is_free === true && data.price_cents !== null && data.price_cents !== undefined) {
        return false;
      }
